...
def set_secondary_config(type_declaration, skeleton, page_name):
    ...
    if type in skeleton[page_name]:
        ...
        for secondary_page_name in secondary_page:
            secondary_page_path = os.path.join(
                os.path.dirname(__file__), secondary_page[secondary_page_name]["page"]
            )
            with open(secondary_page_path) as file:
                ...
                secondary_page_config = remove_auxiliar_fields_deep(
                    secondary_page_config
                )
                skeleton[page_name][secondary_page_name] = secondary_page_config
                type_declaration[page_name][
                    secondary_page_name
                ] = secondary_page_config_type
...
for page_name in skeleton:
    ...
    # Main config in page
    skeleton, type_declaration = set_general_config(
        type_declaration, skeleton, page_name
    )
    # Secondary config in page
    skeleton, type_declaration = set_secondary_config(
        type_declaration, skeleton, page_name
    )
